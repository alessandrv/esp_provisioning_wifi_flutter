...
class Boss {
     ...
     */
    val networks = mutableListOf<WifiNetwork>() // Changed from MutableSet<String> to MutableList<WifiNetwork>
...
class WifiScanManager(boss: Boss) : ActionManager(boss) {
    override fun call(ctx: CallContext) {
        ...
        val conn = boss.connector(name) ?: run {
            ctx.result.error("DEVICE_NOT_FOUND", "Device $name not found", null)
            return
        }
        ...
        boss.connect(conn, proofOfPossession) { esp ->
            ...
            esp.scanNetworks(object : WiFiScanListener {
                override fun onWifiListReceived(wifiList: ArrayList<WiFiAccessPoint>?) {
                    wifiList ?: run {
                        ctx.result.error("NO_WIFI_NETWORKS", "No Wi-Fi networks found", null)
                        return
                    }
                    // Clear previous networks
                    boss.networks.clear()
                    // Populate networks with SSID and RSSI
                    wifiList.forEach {
                        boss.networks.add(WifiNetwork(it.wifiName, it.rssi))
                    }
                    boss.d("scanNetworks: complete ${boss.networks}")
                    Handler(Looper.getMainLooper()).post {
                        // Convert list of WifiNetwork to list of maps
                        val wifiMapList = boss.networks.map { network ->
                            mapOf(
                                "ssid" to network.ssid,
                                "rssi" to network.rssi
                            )
                        }
                        ctx.result.success(wifiMapList)
                    }
                ...
                override fun onWiFiScanFailed(e: java.lang.Exception?) {
                    ...
                    ctx.result.error("WIFI_SCAN_FAILED", "WiFi scan failed", "Exception details: $e")
                }
            })
        }
    }
